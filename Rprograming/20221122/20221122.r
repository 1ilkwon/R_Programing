#작업용 폴더 설정
setwd("C:\\Users\\Yoon\\Rprograming\\20221122")
getwd() #현재 작업 폴더를 설2
print(1+2)
1+2
...
as.numeric('1') +as.numeric('2') # 문자형의 + 연산자as.numeric()

# 결측값 : NA
sum(1,2,3) # sum : 데이터 합
sum(1,2,NA) # NA : null -> 연산이 불가함 , 결측값 연산 = 결과x
sum(1,2,NA,na.rm=T) # NA를 제외하고 합. na.rm=T : NA값을 제외

#Factor 자료형 : 범주를 가지고 있는 자료형 
#                범주를 R에서는 level 이라고도 함
data <- c(3,2,2,1,1,2,3,3,2) # C : 벡터 자료형 
data
factor(data) 
# 범주 : 값의 범위
...
#  factor : vector 의 일종. 범위를 가지고 있는 벡터 객체
