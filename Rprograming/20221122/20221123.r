...
# 문제
# 온라인 쇼핑몰에서 운동화 바지 티셔츠를 구매하려한다.
# 각 상품에 가격은 76,000원 52,000원, 36,000원이다.
# 각 상품에 세일은 5%, 10%, 5%씩 적용된다.
# 3가지 상품을 dataframe에 정의하고, 모든 상품 구매시 결제 금액을 출력하시오.
# 결과 : [결제 금액 : 153200]

item <- c("운동화","바지","티셔츠")
price <-c(76000,52000,36000)
sale <- c(0.95, 0.9, 0.95)
df <- data.frame(item, price, sale)
df
x = subset(df,select=c(price))
x
y = subset(df,select=c(sale))
y
total = x*y
total
money = colSums(total)

sprintf("결제 금액 : %d",money) 
